...
// FETCH DATA
// ==============================
$query = $this->db->query("SELECT * FROM tbl_cash_disbursement_journal ORDER BY journal_id DESC");
$disbursements = $query->getResultArray();

// ==============================
// DASHBOARD CALCULATIONS
// ==============================
$total_disbursements = $this->db->query("SELECT COUNT(*) as total FROM tbl_cash_disbursement_journal")->getRow()->total;
$total_amount = $this->db->query("SELECT COALESCE(SUM(amount),0) as total FROM tbl_cash_disbursement_journal")->getRow()->total;

// ==============================
// FETCH CHART OF ACCOUNTS FOR DROPDOWN (EXPENSE ACCOUNTS)
// ==============================
...
");
$expense_accounts = $coa_query->getResultArray();
...
<div class="me-cashdisbursement-msg"></div>
<input type="hidden" id="__siteurl" data-mesiteurl="<?=site_url();?>" />

<div class="row mb-2">
    <div class="col-12">
        <h4 class="fw-semibold my-3">Cash Disbursement Journal</h4>
...
<div class="row g-3 mb-4">
    <div class="col-md-6">
        <div class="attendance-card">
            <div class="card-body">
                <div>
                    <div class="attendance-label">Total Disbursements</div>
                    <div class="attendance-value"><?=number_format($total_disbursements);?></div>
                    <div class="attendance-sub">All time transactions</div>
                </div>
                <i class="ti ti-cash-banknote attendance-icon"></i>
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="attendance-card">
            <div class="card-body">
                <div>
                    <div class="attendance-label">Total Amount Disbursed</div>
                    <div class="attendance-value">₱<?=number_format($total_amount,2);?></div>
                    <div class="attendance-sub">Total payments made</div>
                </div>
    ...
    <div class="col-12">
        <div class="report-section">
            ...
            <div class="report-actions">
                ...
                <button class="btn-report-action" onclick="showExpenseReport()">
                    <i class="ti ti-chart-pie"></i> Expense
                </button>
...
<div class="row">
    ...
    <div class="col-md-7">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h6 class="fw-semibold mb-0"><i class="ti ti-list me-2"></i>Cash Disbursement Journal</h6>
                <span class="badge bg-light text-dark border"><?=count($disbursements);?> records</span>
            </div>
            <div class="card-body">
                <table id="disbursementTable" class="table table-hover align-middle">
                    <thead>
                        <tr>
                            <th>Date</th>
    ...
    <div class="col-md-5">
        <div class="card">
            <form class="cd-reg-form" id="cdRegForm">
                <div class="card-header">
                    <h6 class="fw-semibold mb-0"><i class="ti ti-plus me-2"></i>Add Cash Disbursement</h6>
                    <small class="text-muted">Record new payment transaction</small>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="payee" class="form-control" name="payee" placeholder="Supplier/Recipient name">
                    </div>
                    <div class="mb-3">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="text-center py-3">
                    ...
                    <p class="text-muted">You are about to delete disbursement: <br>
...
<script src="<?=base_url('assets/js/cashdisbursement/cash-disbursement.js?v=1');?>"></script>
...
function showDailyReport() {
    ...
    if(date) {
        showReport('<?=site_url();?>cash-disbursement-journal?meaction=PRINT-DAILY&report_date=' + date);
    } else {
...
function showSummaryReport() {
    ...
    if(from_date && to_date) {
        showReport('<?=site_url();?>cash-disbursement-journal?meaction=PRINT-SUMMARY&from_date=' + from_date + '&to_date=' + to_date);
    } else {
...
function showJournalReport() {
    ...
    var to_date = document.getElementById('report_to_date').value;
    if(from_date && to_date) {
        showReport('<?=site_url();?>cash-disbursement-journal?meaction=PRINT-JOURNAL&from_date=' + from_date + '&to_date=' + to_date);
    } else {
        toastr.error('Please select both FROM and TO dates');
    }
}
...
function showExpenseReport() {
    ...
    var to_date = document.getElementById('report_to_date').value;
    if(from_date && to_date) {
        showReport('<?=site_url();?>cash-disbursement-journal?meaction=PRINT-EXPENSE&from_date=' + from_date + '&to_date=' + to_date);
    } else {
        toastr.error('Please select both FROM and TO dates');
    }
}
...
function showMonthlyReport() {
    ...
    if(year) {
        showReport('<?=site_url();?>cash-disbursement-journal?meaction=PRINT-MONTHLY&year=' + year);
    } else {
...
function deleteDisbursement() {
    if(deleteId) {
        ...
        jQuery.ajax({
            type: "POST",
            url: '<?=site_url();?>cash-disbursement-journal',
            data: mparam,
            dataType: 'json',

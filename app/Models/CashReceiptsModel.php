...
class CashReceiptsModel extends Model
{
    protected $db;
    ...
    public function saveCashReceipt() { 
        $date = $this->request->getPost('date');
        $account_code = $this->request->getPost('account_code');
        $amount = $this->request->getPost('amount');

        $transaction_id = $this->get_ctr_cashreceipt('CR','CTRL_NO01');
    ...
    public function get_ctr_cashreceipt($tag,$mfld='') { 
		$accessquery = $this->db->query("
		CREATE TABLE if not exists `myctr_cashreceipt` (
		...
		$qctr = $this->db->query("select {$xfield} from myctr_cashreceipt WHERE CTR_YEAR = '$xsysyear' AND CTR_MONTH = '$xsysmonth' AND CTR_DAY = '$xsysday'  limit 1");
		if($qctr->getNumRows() == 0) {
			$xnumb = '001';
			$query = $this->db->query( "insert into myctr_cashreceipt (CTR_YEAR,CTR_MONTH,CTR_DAY,{$xfield}) values('$xsysyear','$xsysmonth','$xsysday','$xnumb')");
			$qctr->freeResult();
		} else {
			$qctr->freeResult();
			$qctr = $this->db->query( "select {$xfield} MYFIELD from myctr_cashreceipt WHERE CTR_YEAR = '$xsysyear' AND CTR_MONTH = '$xsysmonth' AND CTR_DAY = '$xsysday' limit 1");
			...
			if(trim($rctr['MYFIELD'],' ') == '') { 
				...
				$query = $this->db->query("update myctr_cashreceipt set {$xfield} = '{$xnumb}'");
			}
		}
		return  $tag . '-' . $xnumb;
	} 

...
class GeneralJournalModel extends Model
{
    ...
    public function saveEntry() { 
        ...
        $transaction_id = $this->get_ctr_generaljournal('GJ','CTRL_NO01');

        $query = $this->db->query("
            INSERT INTO `tbl_general_journal`(
                `date`,
                `transaction_id`,
                `account_code`,
                `amount`,
    ...
    public function deleteEntry() { 
        ...
        $query = $this->db->query("DELETE FROM `tbl_general_journal` WHERE `journal_id` = ?", [$journal_id]);
    ...
    public function get_ctr_generaljournal($tag,$mfld='') { 
        $accessquery = $this->db->query("
        CREATE TABLE if not exists `myctr_generaljournal` (
        ...
        $qctr = $this->db->query("select {$xfield} from myctr_generaljournal WHERE CTR_YEAR = '$xsysyear' AND CTR_MONTH = '$xsysmonth' AND CTR_DAY = '$xsysday'  limit 1");
        if($qctr->getNumRows() == 0) {
            $xnumb = '001';
            $query = $this->db->query( "insert into myctr_generaljournal (CTR_YEAR,CTR_MONTH,CTR_DAY,{$xfield}) values('$xsysyear','$xsysmonth','$xsysday','$xnumb')");
            $qctr->freeResult();
        } else {
            $qctr->freeResult();
            $qctr = $this->db->query( "select {$xfield} MYFIELD from myctr_generaljournal WHERE CTR_YEAR = '$xsysyear' AND CTR_MONTH = '$xsysmonth' AND CTR_DAY = '$xsysday' limit 1");
            ...
            if(trim($rctr['MYFIELD'],' ') == '') { 
                ...
                $query = $this->db->query("update myctr_generaljournal set {$xfield} = '{$xnumb}'");
            }
        }
        return $tag . '-' . $xnumb;
    } 
